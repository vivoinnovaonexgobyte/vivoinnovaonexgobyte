...
void PrepareShutdown()
{
    ...
    RenameThread("vivoinnovaonexgobyte-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "vivoinnovaonexgobyte (or specifically: privatesend, instantsend, masternode, keepass, mnpayments, mngovernance)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Vivoinnovaonexgobyte specific functionality (Masternodes, PrivateSend, InstantSend, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizevivoinnovaonexgobyteamount=<n>", strprintf(_("Keep N VIOG anonymized (default: %u)"), nAnonymizeVivoinnovaonexgobyteAmount));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Vivoinnovaonexgobyte Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("vivoinnovaonexgobyte-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
/** Sanity checks
 *  Ensure that Vivoinnovaonexgobyte is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetArg("-liquidityprovider", 0) > 0) {
        ...
        mapArgs["-anonymizevivoinnovaonexgobyteamount"] = "999999";
        LogPrintf("AppInit2 : parameter interaction: -liquidityprovider=%d -> setting -anonymizevivoinnovaonexgobyteamount=999999\n", nLiqProvTmp);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vivoinnovaonexgobyte version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize vivoinnovaonexgobyte.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Vivoinnovaonexgobyte Core is shutting down."));
...
#endif
    // Make sure only a single Vivoinnovaonexgobyte process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vivoinnovaonexgobyte Core is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vivoinnovaonexgobyte Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Vivoinnovaonexgobyte Core to complete") << "\n";
    ...
    nAnonymizeVivoinnovaonexgobyteAmount = GetArg("-anonymizevivoinnovaonexgobyteamount", nAnonymizeVivoinnovaonexgobyteAmount);
    nAnonymizeVivoinnovaonexgobyteAmount = std::min(std::max(nAnonymizeVivoinnovaonexgobyteAmount, 2), 999999);
    ...
    LogPrintf("Anonymize Vivoinnovaonexgobyte Amount %d\n", nAnonymizeVivoinnovaonexgobyteAmount);
    ...
    // ********************************************************* Step 11c: update block tip in Vivoinnovaonexgobyte modules
    ...
    // ********************************************************* Step 11d: start vivoinnovaonexgobyte-privatesend thread
